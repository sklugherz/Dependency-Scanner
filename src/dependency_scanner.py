...
class DependencyScanner():
        ...
        for x in unfiltered_dependencies:
            if x[:1] != '#': # skip commented packages
                ...
                if match(r'^[A-Za-z0-9\-_.=]+$', x):  # ensure only valid characters are used
                    ...
                    format_error_message(f'INVALID PACKAGE: {x}')
    ...
    """
        ...
        makes a query for each valid dependecy to the nvd api
        returns a list of vulnerabilities where each index is the vunerability list of that respective dependency
    """
    def get_nvd_database_response(self):
        vulnerabilties = []
        for x in self.dependencies: 
            ...
            if response.status_code == 200:
                vulnerabilties.append(response.json().get('vulnerabilities', []))
            else:
                format_error_message(f'FAILED TO CONNECT TO NVD DATABASE: {str(response.status_code)}')
                return 1
